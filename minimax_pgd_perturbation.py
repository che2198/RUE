...
def get_args():
    ...
    parser.add_argument('--defender', type=str, default='minimax-pgd',
                        choices=['minimax-pgd', 'minimax-pgd-v3'],
                        help='set defender')

    parser.add_argument('--pgd-lr', type=float, default=1)
    parser.add_argument('--pgd-lr-decay-rate', type=float, default=0.1)
    parser.add_argument('--pgd-lr-decay-freq', type=float, default=100)
    ...
    parser.add_argument('--init-noise', action='store_true',
                        help='if select, init def noise at first')
...
def main(args, logger):
    ...
    elif args.defender == 'minimax-pgd-v3':
        defender = attacks.MinimaxPGDDefenderV3(
            radius           = args.pgd_radius,
            steps            = args.pgd_steps,
            lr               = args.pgd_lr,
            lr_decay_rate    = args.pgd_lr_decay_rate,
            lr_decay_freq    = args.pgd_lr_decay_freq,
            random_start     = args.pgd_random_start,
            atk_radius       = args.atk_pgd_radius,
            atk_steps        = args.atk_pgd_steps,
            atk_step_size    = args.atk_pgd_step_size,
    ...
    # would be possibly dropped in future
    if args.init_noise:
